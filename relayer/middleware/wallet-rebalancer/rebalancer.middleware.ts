...
import { WalletToolBox } from "../wallet/walletToolBox";
import { Middleware } from "../../compose.middleware";
import { spawnWalletWorker } from "../wallet/wallet.worker";
...
export function rebalancers(
  ...
  return async (ctx: WalletContext, next) => {
    if (!executeFunction) {
      ctx.logger?.debug(`Initializing rebalancers...`);
      ...
      ctx.logger?.debug(`Initialized rebalancers`);
    }

    ctx.logger?.debug("rebalancers attached to context");
