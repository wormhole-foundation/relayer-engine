...
describe("wormscan-client", () => {
    ...
    test("should list vaas", async () => {
        const vaasResponse = await client.listVaas(8, "6241ffdc032b693bfb8544858f0403dec86f2e1720af9f34f8d65fe574b6238c");
    ...
    test("should work if request fails and then works", async () => {
      ...
      expect(JSON.stringify(vaasResponse.data)).toBe(JSON.stringify(expectedData));
    });

    test("should fail if retry-after is bigger than max delay", async () => {
      server.respondWith(429, { message: "Resource exhausted" }, { "retry-after": "1" });

      const vaasResponse = await client.listVaas(8, "1000", { retries: 1, maxDelay: 500 });
