...
export class WormholeMock {
    ...
    public async start(): Promise<WormholeMockConfig> {
        ...
        this.httpServer = http.createServer(async (req, res) => {
            ...
            if (req.url?.includes("GetSignedVAA")) { // Default response for getSignedVAA from guardian rpc.
            ...
            if (req.url?.includes("api/v1/vaas") && req.url?.includes("pageSize")) { // Wormholescan api.
    ...
    public respondWith(status: number, data: any, headers?: Record<string, string>) {
