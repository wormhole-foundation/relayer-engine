...
const bundle_builder_helper_1 = require("./bundle-builder.helper");
var Environment;
...
class RelayerApp {
    ...
    fetchVaas(opts) {
        const bundle = new bundle_builder_helper_1.VaaBundleBuilder(this.fetchVaa, {
            vaaIds: opts.ids,
    ...
    async fetchVaa(chain, emitterAddress, sequence) {
        const res = await (0, wormhole_sdk_1.getSignedVAAWithRetry)(this.opts.wormholeRpcs, Number(chain), emitterAddress.toString("hex"), sequence.toString(), { transport: grpcWebNodeHttpTransport.NodeHttpTransport() }, 100, 2);
